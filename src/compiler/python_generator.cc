...
bool PrivateGenerator::PrintPreamble(grpc_generator::Printer* out) {
  ...
  if (generate_in_pb2_grpc) {
    ...
    if(config.grpc_tools_version.size() > 0) {
      ...
      out->Print("def _first_version_is_lower(version1, version2):\n");
      {
        IndentScope raii_method_indent(out);
        out->Print(
            "\"\"\"\n"
            ...
            "\"\"\"\n");
        out->Print("version1_list = version1.split('.')\n");
        out->Print("version2_list = version2.split('.')\n");
        out->Print("\nfor i in range(3):\n");
        {
          IndentScope raii_if_indent(out);
          out->Print("if version1_list[i] < version2_list[i]:\n");
          {
            IndentScope raii_return_indent(out);
            out->Print("return True\n");
          }
        }
        {
          IndentScope raii_elif_indent(out);
          out->Print("elif version1_list[i] > version2_list[i]:\n");
          {
            IndentScope raii_return_indent(out);
