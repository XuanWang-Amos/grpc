...
class StackdriverStatsExporter(object):
    ...
    def export_metrics(self, metrics):
        ...
        import threading
        import sys; sys.stderr.write(f"[{threading.current_thread().ident}]>>> 1.calling export_metrics with metrics len={len(metrics)} at {datetime.utcnow()}\n"); sys.stderr.flush()
        for metric in metrics:
            import sys; sys.stderr.write(f"  >>> 1.1metric: {metric}\n"); sys.stderr.flush()
            self.register_metric_descriptor(metric.descriptor)
        import sys; sys.stderr.write(f"[{threading.current_thread().ident}]>>> 2.after register_metric_descriptor, metrics len={len(metrics)}\n"); sys.stderr.flush()
        import sys; sys.stderr.write(f"[{threading.current_thread().ident}]>>> 3.calling create_batched_time_series at {datetime.utcnow()}\n"); sys.stderr.flush()
        ts_batches = self.create_batched_time_series(metrics)
        import sys; sys.stderr.write(f"[{threading.current_thread().ident}]>>> 4.finished calling create_batched_time_series at {datetime.utcnow()}\n"); sys.stderr.flush()
    ...
    def create_batched_time_series(self, metrics,
        ...
        import sys; sys.stderr.write(f"  >>> 3.1calling create_batched_time_series with {len(metrics)} metrics at {datetime.utcnow()}\n"); sys.stderr.flush()
        # for metric in metrics:
        #     self.create_time_series_list(metric)
        # import sys; sys.stderr.write(f"  >>> finished testing at {datetime.utcnow()}\n"); sys.stderr.flush()
    ...
    def create_time_series_list(self, metric):
        import sys; sys.stderr.write(f"    >>> [create_time_series_list]metric: {metric}\n"); sys.stderr.flush()
    ...
    def register_metric_descriptor(self, oc_md):
        ...
        metric_type = self.get_metric_type(oc_md)
        import sys; sys.stderr.write(f"  >>> 1.2 register_metric_descriptor for: {oc_md.name} at {datetime.utcnow()}\n"); sys.stderr.flush()
        with self._md_lock:
            if metric_type in self._md_cache:
                import sys; sys.stderr.write(f"  >>> 1.3 Return from cache at {datetime.utcnow()}\n"); sys.stderr.flush()
        ...
        import sys; sys.stderr.write(f"  >>> 1.3 Created new at {datetime.utcnow()}\n"); sys.stderr.flush()
        return sd_md
